...
def test_start_launch_with_rerun():
...
def test_start_launch_attributes(config):
...
def test_finish_launch(config):

    mock_rps = mock.create_autospec(RPClient)
    mock_context = mock.Mock()
    ba = BehaveAgent(config, mock_rps)
    ba.finish_launch(mock_context, some_key="some_value")
    mock_rps.finish_launch.assert_called_once_with(end_time=mock.ANY, some_key="some_value")
    mock_rps.close.assert_called_once()


def test_skip_finish_launch(config):
...
def test_start_skipped_feature(config):
...
def test_finish_feature(config, tags, expected_status):
    mock_feature = mock.Mock()
...
def test_start_skipped_scenario(config):
    mock_scenario = mock.Mock()
...
def test_start_scenario(config):
    mock_scenario = mock.Mock()
...
@pytest.mark.parametrize("tags,expected_status", [(None, "PASSED"), (["skip"], "SKIPPED")])
def test_finish_scenario(config, tags, expected_status):
    mock_scenario = mock.Mock()
    mock_scenario.tags = tags
...
def test_start_step_step_based(config):
    config.log_layout = LogLayout.STEP
    mock_step = mock.Mock()
...
def test_start_step_nested_based(config):
...
def test_finish_passed_step_step_based(config):
    config.log_layout = LogLayout.STEP
    mock_step = mock.Mock()
    ...
    mock_rps.finish_test_item.assert_called_once_with(
        item_id="step_id", end_time=mock.ANY, status="PASSED", some_key="some_value"
    )


def test_finish_failed_step_step_based(config):
    try:
        ...
        expected_calls = [mock.call(item_id="step_id", time=mock.ANY, level="ERROR", message=expected_msg)]
        mock_rps.log.assert_has_calls(expected_calls)


def test_finish_failed_step_scenario_based(config):
    try:
        raise AssertionError("error!")
    except AssertionError as e:
        e_traceback = sys.exc_info()[2]
...
def test_log_exception_without_message(config):
...
def test_post__log(mock_mime, config):
...
def test_log_scenario_exception_default_message(config):
...
def test_log_scenario_exception(config):
    try:
...
def test_log_cleanup_step_based(scope, item_type, item_id):
...
def test_log_cleanup_scenario_based(config, scope, item_id):
